...
description: 自动化生成"结构化答题技巧·每日一练"小红书爆款图文帖子 docx。围绕公考结构化面试的答题技巧和思路，用上岸过来人+考官视角，按小红书爆款节奏（标题钩子/短句/emoji/普通vs高分对照/避坑提醒/话题标签）产出。基于固定 docx 模板（18 段）+ python-docx 脚本自动替换，从题库索引选题防重复，答题须经 human-writing 去 AI 味儿。触发词：出一期答题技巧、技巧每日一练、答题思路、结构化答题技巧。
...
```
封面钩子标题（≤20 字，数字/反差/悬念/利益四选一，左对齐）
→ 适用题型（无项目符号）
→ 技巧一句话（破题角度，无项目符号，首行缩进 2 字符，颜色 #85120F）
→ 思路步骤拆解（3 步，带圆点项目符号）
→ 蓝色标题"怎么答？"（Heading 2，同"答题思路"）
→ 真题片段：普通答法（段首 🙅♂️）vs 高分答法（段首 👍）（同题对照）
→ 普通答法点评 / 高分答法点评（Markdown 引用块样式 + 浅灰底色 #F2F2F2）
→ 避坑提醒（蓝色标题）+ 避坑内容（加粗下划线）
→ 一句话总结（加粗下划线）
→ 话题标签（endnote text 尾注样式，12pt 小字）
→ [固定引流段，endnote text 样式 + 橙色加粗居中，模板自带，不动]
```
...
[1/4] 读取待写入内容: ...
[2/4] 写入 段[0] 文本框封面标题（前缀改 结构化答题技巧：）
[2.5/4] 写入 13 个段位（正文，段[8] 自动加 🙅♂️、段[10] 自动加 👍）
[3/4] 保存
[4/4] 自动验证 → ✅ 全部通过
```
...
2. 报告内容：
   ...
   - 验证通过项：段数 18 / 4 张图 / 引流段样式 / 封面「结构化答题技巧：」前缀 / 段[7]「怎么答？」蓝色标题
   - 去 AI 味儿自评
...
| 段位 | 内容 | 脚本行为 |
|------|------|----------|
| 段[0] | 封面文本框（"结构化答题技巧："前缀 + 大标题，左对齐） | 替换标题，前缀统一"结构化答题技巧："（2 镜像） |
| 段[1] | "答题思路"蓝色标题（Heading 2） | 不动 |
| 段[2] | 适用题型（无项目符号） | 替换 |
| 段[3] | 技巧一句话（破题，无项目符号，首行缩进 2 字符，颜色 #85120F） | 替换 |
| 段[4-6] | 思路步骤 3 步（带圆点项目符号） | 替换 |
| 段[7] | "怎么答？"蓝色标题（Heading 2） | 不动 |
| 段[8] | 普通答法 | 替换 + 段首自动加 🙅♂️ |
| 段[9] | 普通答法点评（Markdown 引用块样式 + 浅灰底色 #F2F2F2） | 替换 |
| 段[10] | 高分答法 | 替换 + 段首自动加 👍 |
| 段[11] | 高分答法点评（Markdown 引用块样式 + 浅灰底色 #F2F2F2） | 替换 |
| 段[12] | "避坑提醒："蓝色标题（Heading 2） | 替换 |
| 段[13] | 避坑内容（加粗下划线） | 替换 |
| 段[14] | 一句话总结（加粗下划线） | 替换 |
| 段[15] | 话题标签（endnote text 尾注样式，12pt 小字） | 替换 |
| 段[16] | 引流段（endnote text 样式 + 橙色加粗居中） | **不动**（保留原文） |
| 段[17] | 末尾装饰图 | 不动 |
...
3. **文本框改 2 处**：drawing 镜像 + VML fallback，前缀"结构化答题技巧："要 2 处同步
4. **段[7] 是"怎么答？"蓝色标题**：不再强制 pageBreakBefore，普通答法不另起一页
5. **段数恒定 18**：脚本里有校验，超出就报错
6. **JSON 引号嵌套**：用中文""或转义\"，别用 ASCII 双引号嵌套
7. **模板前缀已改**：段[0] 前缀是"结构化答题技巧："，不是"答题技巧："或"结构化每日一练："
8. **段[8]/段[10] emoji 由脚本自动加**：普通答法段首 🙅♂️、高分答法段首 👍，写在 EMOJI_PREFIX 里，别写进 JSON 内容
9. **段[9]/段[11] 点评用 Markdown 引用块**：样式为"Markdown引用"（左边框线 + 缩进 + 深蓝灰文字）
10. **段[13]/段[14] 加粗下划线**：避坑内容 + 一句话总结，脚本替换文本时保留 run 格式
11. **段[15]/段[16] 用 endnote text 尾注样式**：话题标签 + 引流段小字浅灰；引流段 run 级橙色加粗居中仍保留（run 级优先于段落样式）
12. **段[2]/段[3] 无项目符号，段[4-6] 有圆点项目符号**（numId=12）
13. **段[3] 首行缩进 2 字符**：firstLineChars=200, firstLine=640，去掉悬挂缩进（left=0, hanging=0）
14. **段[9]/段[11] 点评段浅灰底色**：w:shd fill=F2F2F2，始终保留在 pPr 里，脚本替换文本不动 shd
15. **段[15] 话题标签 12pt 小字**：run 级字号 Pt(12)，endnote text 样式基于 Normal 16pt，run 级覆盖
...
□ 段数 = 18
□ 4 张图片全在
□ 引流段样式：endnote text + 加粗 + 颜色 #85120F + 居中（段[16]）
□ 段[0] 文本框：2 个镜像 + 前缀"结构化答题技巧："完整
□ 段[7] 为"怎么答？"蓝色标题（Heading 2），无 pageBreakBefore
□ 段[8] 段首有 🙅♂️、段[10] 段首有 👍
□ 段[9]/段[11] 为 Markdown 引用块样式 + 浅灰底色 #F2F2F2
□ 段[13]/段[14] 加粗下划线
□ 段[15]/段[16] 为 endnote text 样式，段[15] 字号 12pt
□ 段[3] 首行缩进 2 字符（firstLineChars=200）
□ 段[2]/段[3] 无项目符号，段[4-6] 有圆点项目符号
□ 旧技巧关键词零残留
□ 新技巧关键词全命中

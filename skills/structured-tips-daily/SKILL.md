...
| skill | `scripts/test_fill_tips_post.py` | 回归测试（改引擎/模板后必跑） |
...
小红书爆款规范（详见 `references/content-strategy.md`）：

- **标题钩子 ≤20 字**（引擎硬上限），必须含数字/反差/悬念/利益之一
- **单段 ≤4 行，行内 ≤30 字**，短句 8-20 字为主
- **emoji 每段 ≤2 个**，限调色板 `❗️ 🙅 ❌ ✅ 📌 💡 ⏰ 🗣`
- **普通 vs 高分对照**是特色：同题两答法 + 点评，读者能直接抄
- **避坑提醒**给 2-5 句"绝对不能说的官话"
- **话题标签 5-8 个**，固定 `#公考面试 #结构化面试 #上岸`，其余按题型补
- 数字必须可核验或有出处，无出处的统计一律不写
